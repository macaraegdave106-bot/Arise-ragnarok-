...
KillTab:AddToggle({Name = "Auto Kill", Default = false, Callback = function(Value)
    AutoKillEnabled = Value
    if Value then spawn(function() while AutoKillEnabled do if IsAlive() then local enemy = GetClosestQuestEnemy() if enemy and enemy.Root then TeleportTo(enemy.Root.CFrame * CFrame.new(0, 0, -2)) end end task.wait(0.1) end end) end
end})
KillTab:AddToggle({Name = "Kill Aura", Default = false, Callback = function(Value) KillAuraEnabled = Value end})
RunService.Heartbeat:Connect(function() if KillAuraEnabled and IsAlive() then local root = GetRootPart() if root then for _, enemy in pairs(GetQuestEnemies()) do if enemy.Root then enemy.Root.CFrame = root.CFrame * CFrame.new(0, 0, -2) end end end end end)

KillTab:AddSection({Name = "⚡ Auto Attack"})
KillTab:AddToggle({Name = "Auto Attack", Default = false, Callback = function(Value)
    AutoAttackEnabled = Value
    if Value then spawn(function() while AutoAttackEnabled do if IsAlive() and #GetQuestEnemies() > 0 then FastAttack() end task.wait(0.05) end end) end
end})
KillTab:AddSlider({Name = "Attack Speed", Min = 1, Max = 50, Default = 10, Increment = 1, Callback = function(Value) AttackSpeed = Value end})

local EnemyLabel = KillTab:AddLabel("Quest Enemies: 0")
local DeadEnemyLabel = KillTab:AddLabel("Dead Enemies: 0")
local AliveLabel = KillTab:AddLabel("Player: Alive ✅")
...
QuestTab:AddToggle({Name = "Auto Quest", Default = false, Callback = function(Value)
    ...
    if Value then
        spawn(function()
            while AutoQuestEnabled do
                ...
                if QuestState == "IDLE" then
                    local npc = GetClosestQuestNPC()
                    if npc then
                        TeleportTo(npc.Root.CFrame * CFrame.new(0, 0, -5))
                        task.wait(0.5)
                        AcceptQuest(npc.Name)
                        local accepted = false
                        for i = 1, 10 do task.wait(0.5) if GetQuestPoint() then accepted = true break end end
                        if accepted then QuestState = "WAITING_QUEST_POINT" else OrionLib:MakeNotification({Name = "Quest", Content = "Retrying...", Time = 1}) end
                    end
                elseif QuestState == "WAITING_QUEST_POINT" then
                    if questPoint then LastQuestPointPosition = questPoint.Position TeleportTo(questPoint.CFrame * CFrame.new(0, 40, 0)) QuestState = "WAITING_SPAWN" end
                elseif QuestState == "WAITING_SPAWN" then
                    for i = 15, 1, -1 do if not AutoQuestEnabled or #GetQuestEnemies() > 0 then break end task.wait(1) end
                    QuestState = "KILLING"
                    ClearSkippedEnemies()
                elseif QuestState == "KILLING" then
                    if #enemies == 0 and not questPoint then QuestState = "ARISE_COLLECT" end
                elseif QuestState == "ARISE_COLLECT" then
                    local deadList = GetDeadEnemies()
                    if #deadList > 0 then
                        for _, dead in pairs(deadList) do
                            if not AutoQuestEnabled or SkippedEnemies[dead.Name] then continue end
                            if AutoTPDeadEnabled and dead.Root then TeleportTo(dead.Root.CFrame * CFrame.new(0, 0, -2)) end
                            local found = false
                            for i = 1, 8 do if HasAriseOrCollect() then found = true break end task.wait(0.25) end
                            if found then
                                while HasAriseOrCollect() and AutoQuestEnabled do ClickAllAriseCollect() task.wait(0.3) end
                            else
                                SkippedEnemies[dead.Name] = true
                            end
                            task.wait(0.3)
                        end
                    end
                    if #GetDeadEnemies() == 0 then
                        QuestState = "IDLE"
                        LastQuestPointPosition = nil
                        ClearSkippedEnemies()
                    end
                end
                task.wait(0.5)
...
QuestTab:AddButton({Name = "Go to Quest", Callback = function() local qp = GetQuestPoint() if qp then TeleportTo(qp.CFrame * CFrame.new(0, 40, 0)) end end})
...
Player.CharacterAdded:Connect(function(char)
    if AutoRespawnEnabled and LastQuestPointPosition then
        task.wait(1.5)
        local root = char:WaitForChild("HumanoidRootPart", 5)
        if root then root.CFrame = CFrame.new(LastQuestPointPosition) * CFrame.new(0, 40, 0) end
    end
end)
...
spawn(function() while task.wait(0.5) do
    ...
    local skip = 0 for _ in pairs(SkippedEnemies) do skip = skip + 1 end SkippedLabel:Set("Skipped: " .. skip)
    NPCLabel:Set("NPCs: " .. #GetQuestNPCs())
    local qp = GetQuestPoint() QuestPointLabel:Set(qp and "Quest: ✅" or "Quest: ❌")
    ...
    if AutoQuestEnabled then
        if QuestState == "IDLE" then StatusLabel:Set("Status: Finding Quest 🔍")
        elseif QuestState == "WAITING_QUEST_POINT" then StatusLabel:Set("Status: Going ⏳")
        elseif QuestState == "WAITING_SPAWN" then StatusLabel:Set("Status: Waiting ⏳")
        elseif QuestState == "KILLING" then StatusLabel:Set("Status: Killing 🔥")
        elseif QuestState == "ARISE_COLLECT" then StatusLabel:Set("Status: Arise/Collect 👻") end
    else StatusLabel:Set("Status: Idle 💤") end
end end)

spawn(function() while task.wait(0.15) do
    if not AutoQuestEnabled and IsAlive() and HasAriseOrCollect() then
        if AutoTPDeadEnabled then local d = GetClosestDeadEnemy() if d and d.Root then TeleportTo(d.Root.CFrame * CFrame.new(0, 0, -2)) end end
        if AutoAriseEnabled or AutoCollectEnabled then ClickAllAriseCollect() end
    end
end end)
